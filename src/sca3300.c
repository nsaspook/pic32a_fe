...
bool sca3300_check_crc(imu_cmd_t * imu, uint8_t option)
{
	...
	default:
		if (CalculateCRC(imu->rbuf32[SCA3300_REC]) != (imu->rbuf32[SCA3300_REC]&0xff)) {
			imu->crc_error = true; // set CRC failure of returned data
			crc = false;
		}
		break;
	}
...
bool sca3300_imu_transfer(imu_cmd_t * imu, uint32_t data)
{
	...
	SPI3BUF = imu->tbuf32[SCA3300_TRM];
...
static void SCCP1_Callback_InterruptHandler(uint32_t status, uintptr_t context)
{
	time_done = true;
}
